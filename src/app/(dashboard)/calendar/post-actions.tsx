...
export function PostActions({
  ...
  const handleDelete = () => {
    if (confirm("Are you sure you want to delete this post?")) {
      alert("Deleted (feature in progress)");
  ...
  const handlePublishNow = () => {
    if (confirm("Are you sure you want to publish this post immediately?")) {
      alert("Publishing request sent (feature in progress)");
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-card text-card-foreground w-full max-w-lg rounded-lg border border-border shadow-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4 max-h-[80vh] overflow-y-auto">
              <div>
                <label className="block text-sm font-medium mb-1">Content (Caption)</label>
                <textarea
            ...
            <div className="flex justify-end gap-2 border-t border-border px-4 py-3">
              ...
              <button
                onClick={() => {
                  alert("Changes saved (feature in progress)");
